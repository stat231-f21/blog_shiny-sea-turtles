...
```{r}
newzeal_map_df <- map_data(map = "nz")

ggplot(newzeal_map_df, aes(x = long, y = lat, group = group)) +
  geom_polygon(fill = "lightgrey", color = "white") +
  # Use empty theme to remove background color, axes, ticks
  theme_void()
```
...
hate_crimes$ID <- tolower(hate_crimes$ID)

hate_crimes_map <- usa_map %>% 
  left_join(hate_crimes)

hate_crimes_map <- hate_crimes_map[-8, ]
  

ggplot(hate_crimes_map, aes(fill = avg_hatecrimes_per_100k_fbi)) +
  geom_sf() +
  scale_fill_viridis(option = "magma", direction = -1) +
  theme_void() +
  labs(fill = "Crimes per 100,000 Population",
       title = "Average Annual Hate Crimes per 100,000 Population",
       subtitle = "2010-2015") +
  theme(legend.position = "bottom")
ggsave(file="hate-crimes.png")
```
