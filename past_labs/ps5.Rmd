...
This Wikipedia page allows automated scraping.
...
write.csv(justices, file = 'justices.csv')
```
...
justices <- read_csv(file = 'data/justices.csv')
...
ggplot(data = justices, 
       mapping = aes(x = tenure_years)) +
  geom_histogram() +
  labs(title = "Tenure Length of U.S. Supreme Court judges",
     x = "Years",
     y = "Number of Judges")
```

The histogram appears to be multimodal and skewed slightly to the right with no apparent outliers and no gaps. As a result, it appears that more judges have tenure lengths of around 5 to 15 years than longer term lengths of 20+ years. 
...
-->
```{r, eval = FALSE}
quotes_url <- "https://www.brainyquote.com/topics/success-quotes"
robotstxt::paths_allowed(quotes_url)
...
# put in data frame with two variables (person and quote)
quotes <- tibble(person = people, quote = quotes) %>% 
  mutate(quote = str_remove_all(quote, "\n\n"),
         # Prep quotes for markdown display when `results = "asis"`
         display = paste0('> *"', as.character(quote), '"* --' , as.character(person)))

write_csv(quotes, "ps5/data/quotes.csv")
```
...
```{r, results = "asis"}
quotes <- read_csv("data/quotes.csv")

slice_sample(quotes, n = 3) %>% 
  pull(display) %>% 
  cat()
```
